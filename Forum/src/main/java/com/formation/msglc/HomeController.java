...
public class HomeController {
	...
	public String log(@Valid Identifiant iden, BindingResult bindingResult, Model model, HttpSession session) {
		...
		if (a == null) {
			model.addAttribute("errorBean", new Message("Email ou mot de passe incorrecte ou bien vous etes Bloque"));
			model.addAttribute("identifiant", iden);
			return "login";
		}
	...
	@RequestMapping(value = "/membre/{idBloquer}", method = RequestMethod.GET)
	public String membre(@PathVariable(value = "idBloquer") Long id, Model model, HttpSession session  ) {
		
		ArrayList<Auteur> utilisateurs = (ArrayList<Auteur>) metier.getAllAuteurs();
	...
	public String membre(Model model, HttpSession session  ) {
