...
<body>
	<div class="divProfil">
		...
		<c:choose>
			<c:when test="${not empty sessionScope.auteur}">
				<a href="${pageContext.servletContext.contextPath}/deconnecter/${post.id}/${post.idPage}">se
					déconnecter</a><br>
				...
				<c:if test="${sessionScope.auteur.role=='ADMIN' || sessionScope.auteur.role=='MODERATOR' }">
					...
					<!--  ajouter par moi lien pour voire tout les membre-->
					<a href="${pageContext.servletContext.contextPath}/membre">Members</a>
				</c:if>
			</c:when>
			<c:otherwise>
